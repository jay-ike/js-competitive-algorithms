...
function findAnagramIndices(word, text) {
    ...
    if (wordLength > textLength) {
        throw new Error(`${word} length should be lesser or equal to ${text} length`);
    }
...
function PalindromePairsIndices(wordList) {
    ...
    for (let index = 0; index < wordList.length; index++) {
        ...
        for (let characterIndex = 0; characterIndex < element.length; characterIndex++) {
...
function zigzagString(string, totalRows) {
    ...
    for (let index = 0; index < totalRows; index++) {
        ...
        while (characterIndex < stringLength) {
            result = `${result}${string[characterIndex]}${getRowSpace(itemPosition, index, totalRows)}`;
            itemPosition += 1;
            if (index === 0 || index === totalRows - 1) {
                characterIndex += 2 * (totalRows - 1);
            } else {
                characterIndex += (itemPosition % 2 === 0) ? 2 * index : 2 * (totalRows - index - 1);
            }
        }
        result = `${result}\n`;
...
function getRowSpace(itemPosition,rowIndex, totalRows) {
    ...
    if (rowIndex === totalRows - 1 || rowIndex === 0) {
        ...
        return ' '.repeat(itemPosition % 2 === 0 ? 2 * (totalRows - rowIndex) - 3 : 2 * rowIndex - 1);
    }
}

function longestPalindromeSubstring(text) {
    let forbiddenTokens = ['#', '^', '$'];
    let center = 1, maxDiameter = 1;
    ...
    for (let index = 2; index < transformedText.length -1; index++) {
        ...
        allRadii[index] = Math.max(0, Math.min(maxDiameter - index, allRadii[mirror]));
        while (transformedText[index-(allRadii[index]+1)]===transformedText[index+allRadii[index]+1]) {
            allRadii[index] += 1;
        }
...
function throwIfContains(tokens, text) {
    ...
    let hasNoToken = tokens.map(e => textCharacters.has(e)).every(e => e === false);
    if (!hasNoToken) {
